...
def train_model(
    ...
    Returns:
        Tuple[Dict, Iterator]: Best model weights and training and validation losses
            through epochs
    """
    ...
    return best_model_wts, zip(*[train_epoch_loss, validation_epoch_loss])
...
def evaluate_model(
    ...
    for i, samples in tqdm(enumerate(data_loader), desc="Evaluating.."):
        images = samples["image_tensors"].to(device)
        gts = samples["labels"]
...
def validate_model(model: torch.nn.Module, config: Dict, classes: List) -> None:
    ...
    # load model with weights in eval mode
    device = torch.device(config["device"] if torch.cuda.is_available() else "cpu")
    model.load_state_dict(torch.load(config["evaluation"]["weights"]))
    model.eval()
    model.to(device)
    # predict and visualize for validation
    preds = predict(model, img_paths, transforms, device, classes)
    print(f'Saving resut images in {config["validated_images_dir"]}..')
